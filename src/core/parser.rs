use std::borrow::Cow;
pub(crate) use crate::core::expr::{ExprKind, LiteralValue, NodeId, AST};
...
impl<'a> Parser<'a>{
    ...
    fn unary(&mut self, ast: &mut AST<'a>) -> Result<NodeId, KilnError> {
        if self.matches(&[TokenType::Bang, TokenType::Minus]){
            let operator = self.previous();
            let right = self.unary(ast)?;
            return Ok(ast.add_node(ExprKind::Unary {
                operator,
                right
            }))
        }
    ...
    fn primary(&mut self, ast: &mut AST<'a>) -> Result<NodeId, KilnError> {
        if self.is_at_end() { return Err(KilnError::Runtime {message: "Invalid token".to_string()})}
        let expr;
        match self.advance().token_type {
            ...
            _ => { return Err(KilnError::Runtime {message: "Expect expression.".to_string()})}
        }
    ...
    fn consume(&mut self, token_type: TokenType<'a>, message: &str) -> Result<Token<'a>, KilnError> {
        let tok = token_type.clone();
        if self.check(&token_type) { return Ok(self.advance())}
        match tok {
            TokenType::Eof => Err(KilnError::Runtime {message: report_error(self.peek().line, Some(" at end"), message)}),
            _ => Err(KilnError::Runtime {message: report_error(self.peek().line, Some(&format!(" at '{}'", self.peek().lexeme)), message)})
        }
    }
    ...
    fn synchronize(&mut self){
        ...
        while !self.is_at_end() {
            ...
            match self.peek().token_type{
                TokenType::Class | TokenType::For | TokenType::Fn | TokenType::If | TokenType::Print
...
#[cfg(test)]
mod test{
    use crate::core::expr::{format_ast, AST};
